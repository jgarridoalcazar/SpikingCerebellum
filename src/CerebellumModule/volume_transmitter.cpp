...
mynest::volume_transmitter::calibrate()
{
  ...
  B_.spikecounter_.reserve( nest::kernel().connection_manager.get_min_delay() * P_.deliver_interval_ + 1  );
}

void
mynest::volume_transmitter::update( const nest::Time& t_orig, const long from, const long to )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    if ( multiplicity > 0 )
    {
      t_spike =
        nest::Time(
          nest::Time::step( nest::kernel().simulation_manager.get_slice_origin().get_steps()
            + lag + 1 ) ).get_ms();
  ...
  if ( ( nest::kernel().simulation_manager.get_slice_origin().get_steps() + to )
      % ( P_.deliver_interval_ * nest::kernel().connection_manager.get_min_delay() )
    == 0 )
  {
    double t_trig =
      nest::Time(
    ...
    if ( !B_.spikecounter_.empty() )
      nest::kernel().connection_manager.trigger_update_weight(
