...
STDPSinCommonProperties::get_status( DictionaryDatum& d ) const
{
  ...
  def< nest::double_t >( d, "Wmin", Wmin_ );
...
STDPSinCommonProperties::set_status( const DictionaryDatum& d, nest::ConnectorModel& cm )
{
  ...
  {
    vt_ = dynamic_cast< mynest::volume_transmitter* >( 
      nest::NestModule::get_network().get_node( vtgid, 
      nest::NestModule::get_network().get_thread_id()));
  ...
  updateValue< nest::double_t >( d, "Wmin", Wmin_ );
