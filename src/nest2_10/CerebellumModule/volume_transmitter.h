...
#ifndef VOLUME_TRANSMITTER_SIN_H
#define VOLUME_TRANSMITTER_SIN_H
...
private:
  ...
  struct Buffers_
  {
    ...
    std::vector< nest::spikecounter > spikecounter_; //!< vector to store and deliver spikes
  };
...
inline nest::port
volume_transmitter::handles_test_event( nest::SpikeEvent&, nest::rport receptor_type )
{
  if ( receptor_type != 0 )
