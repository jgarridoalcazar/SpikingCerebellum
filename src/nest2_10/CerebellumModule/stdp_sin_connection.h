...
private:
  ...
  std::vector<nest::double_t> state_vars_;
  ...
  nest::double_t Peak_;
  nest::uint_t Exponent_;
  nest::double_t inv_tau_;
  nest::double_t factor_;
  nest::float_t * TermPointer_;

  void update_synaptic_state(nest::double_t t_spike);
};
...
STDPSinConnection< targetidentifierT >::STDPSinConnection()
  ...
  , TermPointer_( 0 )
{
  this->state_vars_ = std::vector<nest::double_t>(this->Exponent_+2);
  inv_tau_ = atan((nest::double_t) this->Exponent_)/Peak_;
  factor_ = 1.0f/(exp(-atan((nest::double_t)this->Exponent_))*pow(sin(atan((nest::double_t)this->Exponent_)),(nest::int_t) this->Exponent_));

  nest::uint_t ExponenLine = this->Exponent_/2;
...
STDPSinConnection< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
  ...
  def< nest::double_t >( d, nest::names::weight, this->weight_ );

  def< nest::double_t >( d, "peak", this->Peak_ );
  def< nest::uint_t >( d, "exponent", this->Exponent_ );
}
...
STDPSinConnection< targetidentifierT >::set_status( const DictionaryDatum& d, nest::ConnectorModel& cm )
{
  ...
  nest::long_t expon;
  if ( updateValue< nest::long_t >( d, "exponent", expon ))
  { 
    if (expon<2 || expon>20 || expon%2!=0)
    {
      throw nest::BadProperty( "STDP sin exponent must be an even integer between 2 and 20" );
    }
    this->Exponent_ = (nest::uint_t) expon;

    nest::uint_t ExponenLine = Exponent_/2;
    TermPointer_ = STDPSinCommonProperties::terms[ExponenLine]; 

    this->state_vars_.resize(this->Exponent_+2);
  }

  updateValue< nest::double_t >( d, "peak", this->Peak_ );
  
  this->inv_tau_ = atan((nest::double_t) this->Exponent_)/this->Peak_;
  this->factor_ = 1.0f/(exp(-atan((nest::double_t)this->Exponent_))*pow(sin(atan((nest::double_t)this->Exponent_)),(nest::int_t) this->Exponent_)); 
}

template < typename targetidentifierT >
void STDPSinConnection< targetidentifierT >::update_synaptic_state(nest::double_t t_spike){
  nest::double_t OldExpon = this->state_vars_[1];

  nest::double_t ElapsedTime = nest::double_t(t_spike - this->t_last_update_);
  nest::double_t ElapsedRelative = ElapsedTime*this->inv_tau_;

  nest::double_t expon = ExponentialTable::GetResult(-ElapsedRelative);

  this->t_last_update_ = t_spike;
  
  nest::double_t NewExpon = OldExpon * expon;
  nest::double_t NewActivity =NewExpon*this->TermPointer_[0];

  nest::int_t aux=TrigonometricTable::CalculateOffsetPosition(2*ElapsedRelative);
  nest::int_t LUTindex=0;

  nest::double_t SinVar, CosVar, OldVarCos, OldVarSin, NewVarCos, NewVarSin;
  nest::int_t grade, offset;
  ...
  const STDPSinCommonProperties& cp )
{
  ...
  for (std::vector< nest::spikecounter>::const_iterator it = sin_spikes.begin(); it!=sin_spikes.end(); ++it){
    nest::double_t t_spike = it->spike_time_;
    ...
    //std:: cout << "Triggering update weight at time " << t_trig << " with spike at time " << t_spike << ". Updated weight: " << this->weight_ << std::endl;  
  }
